<%@ taglib prefix="s" uri="/struts-tags"%>

<html>
	<head>
		<title>Import Your App</title>
	</head>

	<body >


<h1>Import Your App to Salesforce - Beta</h1>

<p>Import Your App in three simple steps:</p>

<p>1. Select your Microsoft Excel Spreadsheet based application (XLS format).</p>

<p>2. Click "upload" and wait a few moments while Import Your App creates a matching object and uploads your data.</p>

<p>3. Configure your access permissions to control which profiles can work with your data in setup.</p>

<p>That's it.  <a href="<s:url action="startUpload"><s:param name="s" value="existingSessionId" /><s:param name="u" value="existingLocationUrl" /></s:url>">Click here to get started.</a></p>


	</body>
</html>
